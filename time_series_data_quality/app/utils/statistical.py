...
from utils.test_dataframe import TestDf
...
class Statistical:
    ...
    def get_eval_w_rules(self, df_eval):
        ...
        df_eval['eval_rule_1'] = np.\
            where( np.abs(df_eval.mtr_var - df_eval.mean_var) > 3.0 * df_eval.std_dev_var, 1, 0)

        # Rule 2
        df_eval['eval_rule_2'] = np.\
            where( np.abs(df_eval.mtr_var - df_eval.mean_var) > 2.0 * df_eval.std_dev_var, 1, 0)

        # Rule 3
        df_eval['eval_rule_3'] = np.\
            where( np.abs(df_eval.mtr_var - df_eval.mean_var) > df_eval.std_dev_var, 1, 0)
    ...
    def get_w_rule_1(self, df_eval):
        ...
        w_rule_1_var = pd.DataFrame(df_eval.groupby(["entity", "entity_var"])['eval_rule_1'].\
    ...
    def get_w_rule_2(self, df_eval):
        ...
        w_rule_2_var = pd.DataFrame(df_eval.groupby(["entity", "entity_var"])['eval_rule_2'].\
        agg(lambda x: x.iloc[0:3].tolist()))

        w_rule_2_var['w_rule_2_val'] = w_rule_2_var['eval_rule_2'].apply(lambda x: sum(x))
    ...
    def get_w_rule_3(self, df_eval):
        ...
        w_rule_3_var = pd.DataFrame(df_eval.groupby(["entity", "entity_var"])['eval_rule_3'].\
        agg(lambda x: x.iloc[0:5].tolist()))

        w_rule_3_var['w_rule_3_val'] = w_rule_3_var['eval_rule_3'].apply(lambda x: sum(x))
    ...
    def get_w_rule_4(self, df_eval):
        ...
        w_rule_4_var = pd.DataFrame(df_eval.groupby(["entity", "entity_var"])['eval_rule_4'].\
        agg(lambda x: x.iloc[0:9].tolist()))

        w_rule_4_var['w_rule_4_val'] = w_rule_4_var['eval_rule_4'].apply(lambda x: sum(x))
    ...
    def generate(self):
        ...
        df_result_eval[['w_rule_1_rank',
                        'w_rule_2_rank',
                        'w_rule_3_rank',
                        'w_rule_4_rank']] = df_result_eval[['w_rule_1',
                                                            'w_rule_2',
                                                            'w_rule_3',
                                                            'w_rule_4']].astype(int)
        df_result_eval["eval_rank"] = df_result_eval[['w_rule_1_rank',
                                                      'w_rule_2_rank',
                                                      'w_rule_3_rank',
                                                      'w_rule_4_rank']].\
