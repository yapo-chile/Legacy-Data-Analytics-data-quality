    ...
    def query_athena_ts_ad_phone_number_called(self) -> str:
        ...
        query = """
                ...
                -- Q registros
                    ...
                    WHERE 
                        DATE(SUBSTR(event_date, 1,10)) BETWEEN (SELECT min(day_range) as date_from FROM DATES_RANGE_COLUMN) AND (SELECT max(day_range) as date_to FROM DATES_RANGE_COLUMN)
                        and event_name = 'Ad phone number displayed'
                        and client_id = 'yapocl'
                    GROUP BY 1,2,3
                    UNION ALL
                -- Q event type 
                    SELECT
                        ...
                        event_name as entity, 
                        case
                            when event_type = 'Show' then '02_q_event_type_show'||'-'||product_type
                            when event_type is null then '03_q_event_type_null'||'-'||product_type
                            else '04_q_event_type_others'||'-'||product_type
                        end entity_var,
                        sum(1) as mtr_var
                ...
                -- Q lead type 
                    SELECT
                        ...
                        event_name as entity, 
                        case
                            when lead_type = 'Ad phone number displayed' then '32_q_lead_type_Ad_phone_number_displayed'||'-'||product_type
                            when lead_type is null then '33_q_lead_type_null'||'-'||product_type
                            else '34_q_lead_type_others'||'-'||product_type
                        end entity_var,
                        sum(1) as mtr_var
                ...
                -- Q registros
                    ...
                    WHERE 
                        DATE(SUBSTR(event_date, 1,10)) BETWEEN (SELECT min(day_range) as date_from FROM DATES_RANGE_COLUMN) AND (SELECT max(day_range) as date_to FROM DATES_RANGE_COLUMN)
                        and event_name = 'Ad reply submitted'
                        and client_id = 'yapocl'
                    GROUP BY 1,2,3
                    UNION ALL
                -- Q event type 
                    SELECT
                        ...
                        event_name as entity, 
                        case
                            when event_type = 'Send' then '02_q_event_type_Send'||'-'||product_type
                            when event_type is null then '03_q_event_type_null'||'-'||product_type
                            else '04_q_event_type_others'||'-'||product_type
                        end entity_var,
                        sum(1) as mtr_var
                ...
                -- Q object type 
                    SELECT
                        ...
                        event_name as entity, 
                        case
                            when object_type = 'Message' then '05_q_object_type_Message'||'-'||product_type
                            when object_type is null then '06_q_object_type_null'||'-'||product_type
                            else '07_q_object_type_others'||'-'||product_type
                        end entity_var,
                        sum(1) as mtr_var
                ...
                -- Q lead type 
                    SELECT
                        ...
                        event_name as entity, 
                        case
                            when lead_type = 'Ad reply submited' then '32_q_lead_type_Ad_reply_submited'||'-'||product_type
                            when lead_type is null then '33_q_lead_type_null'||'-'||product_type
                            else '34_q_lead_type_others'||'-'||product_type
                        end entity_var,
                        sum(1) as mtr_var
